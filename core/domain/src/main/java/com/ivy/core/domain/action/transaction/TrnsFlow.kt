 ...
 *
 * ### Filters
 * - [TrnWhere.ByAccountId]
 * - [TrnWhere.ByCategoryId]
 * - [TrnWhere.ByType]
...
class TrnsFlow @Inject constructor(
...
) : com.ivy.core.domain.action.FlowAction<TrnWhere, List<Transaction>>() {
    ...
    private fun transactionType(
        ...
        return when (entity.type) {
            TrnTypeOld.INCOME -> Either.Right(TransactionType.Income)
            TrnTypeOld.EXPENSE -> Either.Right(TransactionType.Expense)
            TrnTypeOld.TRANSFER -> {
                val toAccount =
