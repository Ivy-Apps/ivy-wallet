...
class CustomerJourneyCardsProvider @Inject constructor(
  ...
  companion object {
    ...
    fun pollCard() = CustomerJourneyCardModel(
      id = "poll",
      // to users that haven't passed it
      condition = { lng: Long, lng1: Long, ctx: IvyWalletCtx, _ -> true},
      title = "How much are you willing to pay for Ivy Wallet?",
      description = "Since Google Play requires Ivy Wallet to target API level 35+, we'd like to know if you will be interested to pay on a subscription basis so we can maintain the app.",
      cta = "Vote",
      ctaIcon = R.drawable.ic_arrow_right,
      hasDismiss = false,
      background = Gradient.solid(Ivy),
      // navigate to PollScreen
      onAction = { navigation: Navigation, ctx: IvyWalletCtx, screen: RootScreen -> }
    )
